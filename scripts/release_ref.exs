defmodule AshOaskit.ReleaseRef do
  @moduledoc false

  @sha_pattern ~r/\A[0-9a-f]{40}\z/

  @spec verify!(String.t(), %{String.t() => String.t()}, Path.t(), String.t()) :: :ok
  def verify!(version, environment, repository \\ ".", protected_ref \\ "origin/main") do
    ...
    unless git!(repository, ["cat-file", "-t", expected_ref]) == "tag" do
      raise ArgumentError, "#{expected_ref} must be an annotated tag"
    end
    ...
    unless ancestor?(repository, github_sha, protected_ref) do
      raise ArgumentError,
            "release SHA #{github_sha} is not an ancestor of protected #{protected_ref}"
    end
  ...
  defp resolve_commit!(repository, ref) do
    git!(repository, ["rev-parse", "--verify", "#{ref}^{commit}"])
  end

  defp ancestor?(repository, ancestor, descendant) do
    case System.cmd(
           "git",
      ...
      {output, status} ->
        raise ArgumentError, "git ancestry check failed (#{status}): #{String.trim(output)}"
    end
  end
  ...
  defp git!(repository, arguments) do
    case System.cmd("git", ["-C", repository | arguments], stderr_to_stdout: true) do
      ...
      {output, status} -> raise ArgumentError, "git failed (#{status}): #{String.trim(output)}"
    end
  end
end
